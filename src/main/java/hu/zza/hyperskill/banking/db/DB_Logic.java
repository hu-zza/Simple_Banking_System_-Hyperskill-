...
abstract class DB_Logic {
    ...
    static DB_Reply addAccount(DataBase dataBase, DB_Query dataBaseQuery) {
        ...
        var result = makeUpdate(dataBase.getConnection(),
                                String.format("INSERT INTO card (number, pin, balance) VALUES ('%s', '%s', %d);",
                                              account.getCardNumber(), account.getPinCode(), account.getBalance()));
    ...
    static DB_Reply synchronizeAccount(DataBase dataBase, DB_Query dataBaseQuery) {
        ...
        var result = makeQuery(dataBase.getConnection(),
                               String.format("SELECT * FROM card WHERE id = '%d' AND number = '%s' AND pin = '%s';",
                                             account.getDatabaseId(), account.getCardNumber(), account.getPinCode()));
    ...
    static DB_Reply modifyBalance(DataBase dataBase, DB_Query dataBaseQuery) {
        ...
        var result = makeUpdate(dataBase.getConnection(), String.format(
                "UPDATE card SET balance = balance + %d WHERE id = %d AND number = '%s' AND pin = '%s';", amountToAdd,
                account.getDatabaseId(), account.getCardNumber(), account.getPinCode()));
    ...
    static DB_Reply doTransfer(DataBase dataBase, DB_Query dataBaseQuery) {
        ...
        reply = isExist(dataBase, new DB_Query(TransactionType.CHECK_ACCOUNT_EXISTENCE,
        ...
        var result = makeUpdate(dataBase.getConnection(), String.format(
                ...
                + "UPDATE card SET balance = balance + %1$d WHERE number = '%s';", amountToTransfer,
                ownerAccount.getDatabaseId(), ownerAccount.getCardNumber(), ownerAccount.getPinCode(),
    ...
    private static DB_Reply checkAvailableFunds(DataBase dataBase, Account account, int requiredAmount) {
        var result = makeQuery(dataBase.getConnection(),
                               String.format("SELECT * FROM card WHERE id = %d AND number = '%s';",
                                             account.getDatabaseId(), account.getCardNumber()));
